...
db.__proto__.execQuery = function (query, useLogger) {
...
db.__proto__.getIdLastRecordInserted = function () {
    var result = db.execQuery('SELECT last_insert_rowid()', false);
...
var Base = /** @class */ (function () {
    ...
    Base.prototype.insert = function (model) {
        model = model || this;
        queryBuilder.insert(model).into(this.tableName()).run();
        model.id = db.getIdLastRecordInserted();
        return model.id;
    };
    Base.prototype.update = function (model) {
        queryBuilder.table(this.tableName()).update(model).where('id', model.id).run();
        return model.id;
    };
...
var Model = /** @class */ (function (_super) {
    ...
    Model.prototype.getParent = function (model) {
        ...
        var relatedModels = queryBuilder(relatedTable).where('id', idColValue).run();
    ...
    Model.prototype.getChildren = function (model) {
        ...
        try {
            var relatedModels = queryBuilder(childTable).where(fkColumnChildTable, this.id).run();
            return relatedModels.map(function (obj) { return Model.create(obj, model); });
        }
        catch (exception) {
...
var Collection = /** @class */ (function (_super) {
    ...
    Collection.prototype.createTable = function (tableName, columns) {
        var _this = this;
        var createTableQuery = queryBuilder.schema.raw('PRAGMA foreign_keys=ON')
    ...
    Collection.deleteTable = function (tableName) {
        db.runQuery(queryBuilder.schema.dropTableIfExists(tableName));
    };
    return Collection;
}(Base));
